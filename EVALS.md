...
| `12-react-draggable` | real npm package `react-draggable@4.7.2` (`<Draggable>` without `nodeRef`) | third real third-party package; unlike every fixture above, the failure/fix here isn't reachable on mount — `<DraggableCore>`'s own `findDOMNode()` wrapper swallows the missing `ReactDOM.findDOMNode` into a `null` return instead of throwing directly, and the actual crash (`<DraggableCore> not mounted on DragStart!`, [react-draggable#670](https://github.com/react-grid-layout/react-draggable/issues/670)) only surfaces on a real pointer interaction. `scripts/eval-fixture.mjs` drives a real mousedown→mousemove→mouseup sequence via Playwright's mouse API against any fixture element marked `data-drag-target` (a no-op for every other fixture, which has none) rather than the page-side `dispatchEvent` sequence tried first — that didn't reliably reach every listener the library attaches; a real driven interaction did |
